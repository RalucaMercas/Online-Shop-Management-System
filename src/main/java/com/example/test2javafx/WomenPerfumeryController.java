...
public class WomenPerfumeryController implements Initializable {
    @FXML
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        if (productListView != null) {
            if(currentUser.getRole() == UserRole.CLIENT) {
                productListView.getItems().addAll(DBUtils.getVisibleProductsInStockFromBD(4));
            } else {
                if(currentUser.getRole() == UserRole.ADMIN) {
                    productListView.getItems().addAll(DBUtils.getAllVisibleProductsFromBD(4));
                }
            }
        ...
        button_remove_product.setOnAction(event -> {
            ...
            if (!selectedProducts.isEmpty()) {
                ...
                if (result.isPresent() && result.get() == ButtonType.OK) {
                    ...
                    updateAdminWomenPerfumeryProductListView();
                }
            } else {
    ...
    private void updateAdminWomenPerfumeryProductListView() {
        if (productListView != null) {
